...
const SessionCard = ({ 
  ...
  return (
    <div className={`bg-white rounded-lg border shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden relative animate-fade-in group ${
      ...
      <div className="p-5 pb-4">
        <div className="flex items-center mb-4">
          ...
          <div className="ml-4 flex-1 min-w-0">
            <h3 className="text-base font-semibold text-gray-900 mb-1 truncate">{session.name || 'Unnamed Session'}</h3>
            {session.actual_phone ? (
              <div className="flex items-center text-sm font-medium text-gray-700">
                <i className="fas fa-phone mr-2 text-green-600"></i>
                <span className="truncate">{session.actual_phone?.split('@')[0] || 'Not connected'}</span>
              </div>
            ) : (
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium border ${
            ...
            <i className={`fas ${isLoggedIn ? 'fa-shield-check' : 'fa-exclamation-triangle'} mr-2 text-xs`}></i>
          ...
          <div className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium border ${
            ...
            {session.webhook_url ? (
              ...
              <WebhookOff className="w-3 h-3 mr-2" />
            )}
            {session.webhook_url ? 'Webhook Active' : 'No Webhook'}
          </div>
        </div>
      </div>
